...
data <- CLA %>%
  group_by(studentid) %>%
   filter(subject == params$dept, !is.na(score_total)) %>% 
...
data <-CLA %>%
  group_by(studentid) %>%
   filter(subject == params$dept, !is.na(mastery)) %>% 
   filter(n()>1) %>% 
...
if(params$dept == "PSYC")
{
  data <- CLA %>%
   group_by(studentid) %>% 
   filter(subject == params$dept, !is.na(sr_sqr),!is.na(sr_ca),!is.na(sr_cre)) %>%
   filter(n()>1) %>% 
   ungroup %>% 
  mutate(project_year = as.numeric(project_year)) %>%
...
if(nrow(CAT %>%
        ...
        max <- data %>% 
          ...
          group_by(score) %>% tally %>% ungroup %>% select(n) %>% max() 
          
        
        ggplot(data, aes(x = factor(trunc(score)))) +
        geom_bar(aes(fill = fill)) +
        geom_hline(yintercept = seq(0, max, 2), col = "white", lwd = 1) +
        coord_flip() +
        xlab("\nCAT Score") +
...
if(params$dept == "PSYC")
{
  data <- VALUE %>%
   group_by(studentid) %>% 
   filter(subject == params$dept, level!="Not Assessed") %>%
   filter(n()>1) %>% 
   ungroup %>% 
   mutate(project_year = as.numeric(project_year),
         semester = ifelse(course %in% c(101, 100, 104), 1, ifelse(course == 103, 2, ifelse(params$dept %in% c("MINE","CIVL","MTHE","ENPH","ECE","MECH","DRAM") & course %in% c(200, 203), 3, 4)))) %>% 
...
build_slopegraph(summary.data, x="semester", y="level", group="rubric", method="tufte", min.space=0.05) %>% 
  mutate(x = factor(x, levels = c(1,2,3,4), labels = str_wrap(c("Fall 2013-2014","Winter 2014-2015","Fall 2014-2015","Winter 2015-2016"), 6)),
         y = round(y,1),
...
{
  data <- VALUE %>%
   filter(subject == params$dept, !is.na(level)) %>%
   ...
   select(-c(course, subject, consent, discipline, `first year discipline`)) 
} else {
  data <- VALUE %>%
...
div <- ifelse(max> 100,25,ifelse(max>=50,10, 5))
...
ggplot(subset(data, rubric == "Critical Thinking"), aes(x = factor(level))) +
  ...
  geom_hline(yintercept = seq(0, max, div), col = "white", lwd = 1) +
  facet_grid(dimension ~ semester, labeller = label_wrap_gen(width=10)) +
  theme_tufte(base_size = 20) +
  theme(legend.position = "none",
        strip.text.y = element_text(angle = 0),
        axis.title.y = element_text(angle = 0)) +
  scale_y_continuous(limits = c(0, round_any(max,div))) +
...
count <- data %>% group_by(studentid) %>% select(studentid) %>% distinct %>% nrow()
